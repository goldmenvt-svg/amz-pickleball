...
function segmentTopLevel(s, dialect) {
  ...
  while (i < n) {
    ...
    if (!inSingle && !inDouble) {
      ...
      if (c === '&' && dialect === 'cmd') { segments.push(cur); cur = ''; i += 1; continue; }
    }
    cur += c;
    i += 1;
  }
...
function tryStripWrapper(segment) {
  const s = segment.trim();
  let m;
  if ((m = /^env\s+(.+)$/i.exec(s))) return { seg: m[1] };
  if ((m = /^command\s+(.+)$/i.exec(s))) return { seg: m[1] };
  if ((m = /^timeout\s+(?:-\S+\s+)*[0-9.]+[smhd]?\s+(.+)$/i.exec(s))) return { seg: m[1] };
  if ((m = /^time\s+(.+)$/i.exec(s))) return { seg: m[1] };
  if ((m = /^nice\s+(?:-n\s*\S+\s+|-\S+\s+)*(.+)$/i.exec(s))) return { seg: m[1] };
  if ((m = /^nohup\s+(.+)$/i.exec(s))) return { seg: m[1] };
  if ((m = /^stdbuf\s+(?:-\S+\s+)*(.+)$/i.exec(s))) return { seg: m[1] };
  if ((m = /^corepack\s+(.+)$/i.exec(s))) return { seg: m[1] };

  if (/^cmd(?:\.exe)?\s+\/k\b/i.test(s)) return { ambiguous: true };
  if ((m = /^cmd(?:\.exe)?\s+\/s\s+\/c\s+(.+)$/i.exec(s))) return { seg: unquoteOnce(m[1]), dialect: 'cmd' };
  if ((m = /^cmd(?:\.exe)?\s+\/c\s+(.+)$/i.exec(s))) return { seg: unquoteOnce(m[1]), dialect: 'cmd' };
...
function classifyNestedClaude(rest) {
  ...
  const ssIdx = tokens.indexOf('--setting-sources');
  if (ssIdx !== -1 && tokens[ssIdx + 1]) {
    const sources = tokens[ssIdx + 1].split(',').map((x) => x.trim());
    if (sources.indexOf('project') === -1) return denyResult(RULE.TAMPER);
  }

  const settingsIdx = tokens.indexOf('--settings');
  if (settingsIdx !== -1 && tokens[settingsIdx + 1]) {
    const arg = tokens[settingsIdx + 1];
    let parsed = null;
    try { parsed = JSON.parse(arg); } catch (e) { parsed = null; }
    if (parsed && typeof parsed === 'object') {
      ...
      if (parsed.permissions && Array.isArray(parsed.permissions.deny) && parsed.permissions.deny.length === 0) return denyResult(RULE.TAMPER);
      if (parsed.permissions && Array.isArray(parsed.permissions.ask) && parsed.permissions.ask.length === 0) return denyResult(RULE.TAMPER);
      return askResult(RULE.TAMPER);
    }
    return askResult(RULE.TAMPER);
  }
...
function classifyGitConfig(tokens) {
...
function classifyGitRemote(tokens) {
  const sub = tokens[0];
  if (sub && /^(set-url|add|remove|rename|rm)$/i.test(sub)) return denyResult(RULE.TAMPER);
  return deferResult();
}
...
function classifyFirebase(rest) {
  ...
  let i = 0;
  while (i < tokens.length) {
    if (tokens[i] === '--project') { i += 2; continue; }
...
function classifyPackageManager(bin, rest, ctx) {
  ...
  let prefixPath = null;
  while (i < tokens.length) {
    if (bin === 'npm' && tokens[i] === '--prefix') { prefixPath = tokens[i + 1]; i += 2; continue; }
    if (bin === 'pnpm' && tokens[i] === '-C') { prefixPath = tokens[i + 1]; i += 2; continue; }
    if (tokens[i].startsWith('-')) { i += 1; continue; }
    break;
  }
  const subcommand = tokens[i];
...
function classifyEffectiveBinary(segment, dialect, ctx) {
  ...
  const redir = classifyRedirectionTamper(segment, ctx);
...
function classifyCommandString(raw, initialDialect, ctx) {
  ...
  const seg = segmentTopLevel(raw, initialDialect);
